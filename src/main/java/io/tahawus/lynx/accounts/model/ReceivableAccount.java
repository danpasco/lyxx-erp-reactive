...
/**
 * ReceivableAccount - Customer Accounts Receivable subsidiary.
 *
 * Implements Account interface for A/R postings.
 *
 * FORMAT: TT.GG.AAAA.SS
 * Example: "10.15.0100.01" (Assets > Current Assets > A/R Control > ABC Corp)
 *
 * BUSINESS RULES:
 * - Must have controlling GeneralLedgerAccount with subsidiaryType = RECEIVABLE
 * - One ReceivableAccount per customer per controlling account
...
@Table(
        ...
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_receivable_subsidiary_number",
                        columnNames = {"controlling_account_id", "subsidiary_number"}
                ),
                @UniqueConstraint(
                        name = "uk_receivable_contact",
                        columnNames = {"controlling_account_id", "contact_id"}
                ),
                @UniqueConstraint(
                        name = "uk_receivable_short_code",
                        columnNames = {"controlling_account_id", "short_code"}
                )
        },
        indexes = {
                @Index(name = "idx_receivable_controlling", columnList = "controlling_account_id"),
                @Index(name = "idx_receivable_contact", columnList = "contact_id"),
                @Index(name = "idx_receivable_short_code", columnList = "short_code")
        }
)
public class ReceivableAccount extends AuditableEntity implements Account {
    ...
    public String getDisplayName() {
        return shortCode + " - " + contact.name + " (A/R)";
    }
    ...
    protected void validate() {
        if (subsidiaryNumber == null || subsidiaryNumber < 0 || subsidiaryNumber > 99) {
            throw new IllegalStateException("ReceivableAccount subsidiaryNumber must be between 0 and 99");
        }
    }
    ...
    public static List<ReceivableAccount> listByBusiness(Business business) {
        return list("controllingAccount.business = ?1 and isActive = true order by controllingAccount.accountNumber, subsidiaryNumber",
                business);
    }

    public static List<ReceivableAccount> listByControllingAccount(GeneralLedgerAccount controllingAccount) {
        return list("controllingAccount = ?1 and isActive = true order by subsidiaryNumber", controllingAccount);
    }

    public static List<ReceivableAccount> listByContact(Contact contact) {
        return list("contact = ?1 and isActive = true", contact);
    }

    public static Optional<ReceivableAccount> findByShortCode(Business business, String shortCode) {
        return find("controllingAccount.business = ?1 and shortCode = ?2 and isActive = true", business, shortCode)
                .firstResultOptional();
    }

    public static List<ReceivableAccount> search(Business business, String query) {
